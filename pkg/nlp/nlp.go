...
func ModLink(link netlink.Link, add bool) int {
	var ifMac [6]byte
	var ret int
	var err error
	var mod string
	...
	state := uint8(attrs.OperState) != netlink.OperDown
	if add {
		mod = "ADD"
	} else {
		mod = "DELETE"
	...
	if attrs.MasterIndex > 0 {
		brLink, err = netlink.LinkByIndex(attrs.MasterIndex)
		if err != nil {
			fmt.Println(err)
			return -1
		}
	...
	if attrs.MasterIndex > 0 {
		...
		if strings.Contains(name, ".") {
			...
			mif, err := netlink.LinkByIndex(attrs.MasterIndex)
			if err != nil {
				fmt.Println(err)
				return -1
			} else {
				if _, ok := mif.(*netlink.Bond); ok {
	...
	if vxlan, ok := link.(*netlink.Vxlan); ok {
		...
		tunId = vxlan.VxlanId
		uif, err := netlink.LinkByIndex(vxlan.VtepDevIndex)
		if err != nil {
			fmt.Println(err)
			return -1
		}
	...
	} else if _, ok := link.(*netlink.Bond); ok {
		pType = cmn.PortBond
		tk.LogIt(tk.LogInfo, "[NLP] Bond %v, %s\n", name, mod)
	} else if iptun, ok := link.(*netlink.Iptun); ok {
		pType = cmn.PortIPTun
...
func AddAddr(addr netlink.Addr, link netlink.Link) int {
	var ret int
...
func GetLinkNameByIndex(index int) (string, error) {
	brLink, err := netlink.LinkByIndex(index)
	if err != nil {
		return "", err
	}
...
func AddNeigh(neigh netlink.Neigh, link netlink.Link) int {
	var ret int
	var brId int
	var mac [6]byte
	var brMac [6]byte
	...
	if neigh.Family == unix.AF_INET ||
		...
		if neigh.MasterIndex > 0 {
			brLink, err := netlink.LinkByIndex(neigh.MasterIndex)
			if err != nil {
				fmt.Println(err)
				return -1
			}
...
func DelNeigh(neigh netlink.Neigh, link netlink.Link) int {
	var ret int
	var mac [6]byte
	var brMac [6]byte
	var brId int
...
func AddRoute(route netlink.Route) int {
	var ipNet net.IPNet
	if route.Dst == nil {
		r := net.IPv4(0, 0, 0, 0)
		m := net.CIDRMask(0, 32)
	...
	if err != nil {
		...
		if route.Gw != nil {
			tk.LogIt(tk.LogDebug, "[NLP] RT  %s via %s added\n", ipNet.String(),
				route.Gw.String())
		} else {
...
func DelRoute(route netlink.Route) int {
	var ret int
	var ipNet net.IPNet
	if route.Dst == nil {
		r := net.IPv4(0, 0, 0, 0)
...
func AUWorkSingle(m netlink.AddrUpdate) int {
	var ret int
...
func NUWorkSingle(m netlink.NeighUpdate) int {
	var ret int
...
func LUWorker(ch chan netlink.LinkUpdate, f chan struct{}) {
...
func AUWorker(ch chan netlink.AddrUpdate, f chan struct{}) {
...
func NUWorker(ch chan netlink.NeighUpdate, f chan struct{}) {
...
func RUWorker(ch chan netlink.RouteUpdate, f chan struct{}) {
...
func NlpGet(ch chan bool) int {
	...
	tk.LogIt(tk.LogInfo, "[NLP] netlink get done\n")
	ch <- true
	return ret
}
...
func NlpInit() *NlH {
	...
	nNl.FromNUCh = make(chan netlink.NeighUpdate, cmn.NuWorkQLen)
	...
	err := netlink.LinkSubscribe(nNl.FromLUCh, nNl.FromLUDone)
	if err != nil {
		tk.LogIt(tk.LogError, "%v", err)
	} else {
		tk.LogIt(tk.LogInfo, "[NLP] Link msgs subscribed\n")
	}
	err = netlink.AddrSubscribe(nNl.FromAUCh, nNl.FromAUDone)
	if err != nil {
		fmt.Println(err)
	} else {
		tk.LogIt(tk.LogInfo, "[NLP] Addr msgs subscribed\n")
	}
	err = netlink.NeighSubscribe(nNl.FromNUCh, nNl.FromNUDone)
	if err != nil {
		fmt.Println(err)
	} else {
